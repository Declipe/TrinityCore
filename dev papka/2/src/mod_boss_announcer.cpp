...
int32   BossHistoryNormalText   = 31006;
int32   BossHistoryHeroicText   = 31007;
...
public:
    ...
    {
        BossHistoryEnable = sConfigMgr->GetOption<bool>("BossHistory.Enable", false);

        if (!BossHistoryEnable)
            return;

        BossHistoryInstance = sConfigMgr->GetOption<bool>("BossHistory.InstanceOnly", false);
        BossHistoryAnnounce = sConfigMgr->GetOption<bool>("BossHistory.Announce", false);
        BossHistoryLog = sConfigMgr->GetOption<bool>("BossHistory.Log", false);
        BossHistoryNormalText = sConfigMgr->GetOption<int32>("BossHistory.NormalTextId", 31006);
        BossHistoryHeroicText = sConfigMgr->GetOption<int32>("BossHistory.HeroicTextId", 31007);
    }
};
...
class mod_BossHistory_AllCreatureScript : public AllCreatureScript
{
	...
	void AllCreatureJustDied(Creature* creature)
	{
		...
		if(Group *pGroup = recipient->GetGroup())
		{
			for(GroupReference *itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
			{
				...
				if (BossHistoryLog)
				{
					...
					PeopleData << Temp->GetGUID() << DATA_SEPARATOR;
		...
		else
		{
			...
			{
				std::ostringstream PeopleData;
				PeopleData << recipient->GetGUID() << DATA_SEPARATOR;
