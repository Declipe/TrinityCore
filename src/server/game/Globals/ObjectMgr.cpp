...
void ObjectMgr::LoadRandomItemStats()
{
    uint32 oldMSTime = getMSTime();
    //                                                0      1           2
    QueryResult result = ZynDatabase.Query("SELECT id, stat_count, stat_amount FROM random_item_stats;");
    if (!result)
    {
    ...
    TC_LOG_INFO("server.loading", ">> Loaded {} random item stats definitions in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplates2()
{
    uint32 oldMSTime = getMSTime();
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 creature template definitions. DB table `creature_template2` is empty.");
        return;
    }
...
void ObjectMgr::LoadCreatureTemplate2(Field* fields)
{
    uint32 entry = fields[0].GetUInt32();
    CreatureTemplate& creatureTemplate = _creatureTemplateStore[entry];
    ...
    creatureTemplate.SpellSchoolImmuneMask = fields[61].GetUInt32();
    creatureTemplate.flags_extra = fields[62].GetUInt32();
    //creatureTemplate.ScriptID = sObjectMgr->GetScriptId(fields[63].GetString());
    creatureTemplate.ScriptID = GetScriptId(fields[63].GetString());
    creatureTemplate.StringId              = fields[64].GetString();
}
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    for (uint32 diff = 0; diff < MAX_DIFFICULTY - 1 && ok; ++diff)
    {
        ...
        for (uint32 diff2 = 0; diff2 < MAX_DIFFICULTY - 1 && ok2; ++diff2)
        {
            ...
            {
                TC_LOG_INFO("sql.sql", "Creature (Entry: {}) already listed as `difficulty_entry_{}` for another entry.", cInfo->DifficultyEntry[diff], diff2 + 1);
                // TC_LOG_ERROR("sql.sql", "UPDATE `creature_template` SET `difficulty_entry_`= 0 WHERE `entry`={};",
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create Level Stats Data...");
    {
        ...
        do
        {
            ...
            uint32 current_level = fields[2].GetUInt64();
            if (current_level > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
...
bool ObjectMgr::LoadTrinityStrings()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT entry, content_default, content_enGB, content_loc1, content_loc2, content_loc3, content_enCN, content_loc4, content_enTW, content_loc5, content_loc6, content_loc7, content_loc8, content_ptPT, content_ptBR, content_itIT FROM trinity_string");
    if (!result)
    {
...
void ObjectMgr::LoadScriptNames()
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "UNION "
        "SELECT DISTINCT(ScriptName) FROM creature_template2 WHERE ScriptName <> '' "
        "UNION "
        "SELECT DISTINCT(ScriptName) FROM gameobject WHERE ScriptName <> '' "
        "UNION "
        "SELECT DISTINCT(ScriptName) FROM gameobject_template WHERE ScriptName <> '' "
        "UNION "
        "SELECT DISTINCT(ScriptName) FROM item_template2 WHERE ScriptName <> '' "
        "UNION "
...
void ObjectMgr::SendServerMail(Player* player, uint32 id, uint32 reqLevel, uint32 reqPlayTime, uint32 rewardMoneyA, uint32 rewardMoneyH, uint32 rewardItemA, uint32 rewardItemCountA, uint32 rewardItemH, uint32 rewardItemCountH, std::string subject, std::string body, uint8 active) const
{
    if (active)
    {
        ...
        TC_LOG_DEBUG("entities.player", "ObjectMgr::SendServerMail() Sent mail id {} to {}", id, player->GetGUID().ToString().c_str());
    }
}
...
void ObjectMgr::LoadMailServerTemplates()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_server_template` has reqLevel {} but max level is {} for id {}, skipped.", servMail.reqLevel, sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL), servMail.id);
            return;
        }

        if (servMail.moneyA > MAX_MONEY_AMOUNT || servMail.moneyH > MAX_MONEY_AMOUNT)
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_server_template` has moneyA {} or moneyH {} larger than MAX_MONEY_AMOUNT {} for id {}, skipped.", servMail.moneyA, servMail.moneyH, MAX_MONEY_AMOUNT, servMail.id);
            return;
        }
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_server_template` has invalid item in itemA {} for id {}, skipped.", servMail.itemA, servMail.id);
            return;
        }
        ItemTemplate const* itemTemplateH = sObjectMgr->GetItemTemplate(servMail.itemH);
        if (!itemTemplateH && servMail.itemH)
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_server_template` has invalid item in itemH {} for id {}, skipped.", servMail.itemH, servMail.id);
            return;
        }

        if (!servMail.itemA && servMail.itemCountA)
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_server_template` has itemCountA {} with no ItemA, set to 0", servMail.itemCountA);
            servMail.itemCountA = 0;
        }
        if (!servMail.itemH && servMail.itemCountH)
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_server_template` has itemCountH {} with no ItemH, set to 0", servMail.itemCountH);
            servMail.itemCountH = 0;
        }
    } while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded {} Mail Server Template in {} ms", _serverMailStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
