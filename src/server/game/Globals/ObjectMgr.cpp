...
void ObjectMgr::LoadItemTemplatess()
{
    uint32 oldMSTime = getMSTime();
    ...
    //                                           134        135            136
                                             "FoodType, minMoneyLoot, maxMoneyLoot, flagsCustom FROM item_templates");

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 item templates. DB table `item_templates` is empty.");
        return;
    }
    ...
    for (std::set<uint32>::const_iterator itr = notFoundOutfit.begin(); itr != notFoundOutfit.end(); ++itr)
        TC_LOG_ERROR("sql.sql", "Item (Entry: %u) does not exist in `item_templates` but is referenced in `CharStartOutfit.dbc`", *itr);
