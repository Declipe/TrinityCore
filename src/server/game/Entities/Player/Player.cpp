...
void Player::UpdateDamageDoneMods(WeaponAttackType attackType, int32 skipEnchantSlot /*= -1*/)
{
    ...
    for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
    {
        ...
        SpellItemEnchantmentEntry const* enchantmentEntry = sDBCMgr->GetSpellItemEnchantmentEntry(item->GetEnchantmentId(EnchantmentSlot(slot)));
        if (!enchantmentEntry)
            continue;
    ...
    for (int i = KEYRING_SLOT_START; i < CURRENCYTOKEN_SLOT_END; ++i)
        if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i))
            if (pItem != skipItem)
                if (ItemTemplate const* pProto = pItem->GetTemplate())
                    if (pProto->GetItemLimitCategory() == limitCategory)
                        count += pItem->GetCount(); //9475 stroka gitextension
                ...
                if (itemTemplate->Class == ITEM_CLASS_WEAPON || itemTemplate->Class == ITEM_CLASS_ARMOR)
                    if (itemTemplate->Quality == ITEM_QUALITY_UNCOMMON || itemTemplate->Quality == ITEM_QUALITY_RARE || itemTemplate->Quality == ITEM_QUALITY_EPIC)
                            ...
                            itemTemplate->InventoryType == INVTYPE_CLOAK || itemTemplate->InventoryType == INVTYPE_2HWEAPON || itemTemplate->InventoryType == INVTYPE_WEAPONMAINHAND || itemTemplate->InventoryType == INVTYPE_WEAPONOFFHAND || itemTemplate->InventoryType == INVTYPE_RELIC || itemTemplate->InventoryType == INVTYPE_RANGEDRIGHT || itemTemplate->InventoryType == INVTYPE_THROWN)
                        {
                            ...
                            item->SetItemRandomProperties(propertiesList[urand(0, count)]);
                        }
            }
        }
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
        ...
        for (uint32 i = 0; i < quest->GetRewItemsCount(); ++i)
        {
            if (uint32 itemId = quest->RewardItemId[i])
            {
                ...
                if (CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemId, quest->RewardItemCount[i]) == EQUIP_ERR_OK)
                {
                    ...
                    sScriptMgr->OnQuestRewardItem(this, item, quest->RewardItemCount[i]);
                }
    ...
    {
        if (CharTitlesEntry const* titleEntry = sDBCMgr->GetCharTitlesEntry(quest->GetRewTitle()))
            SetTitle(titleEntry);
    }
...
bool Player::LoadFromDB(ObjectGuid guid, CharacterDatabaseQueryHolder const& holder)
{
    ...
    m_extraBonusTalentCount = fields[73].GetUInt8();
    InitTalentForLevel();
    LearnDefaultSkills();
    LearnCustomSpells();
...
bool Player::_LoadHomeBind(PreparedQueryResult result)
{
    ...
    if (!info)
    {
        TC_LOG_ERROR("entities.player", "Player::_LoadHomeBind: Player '{}' ({}) has incorrect race/class ({}/{}) pair. Can't load.",
            GetGUID().ToString(), GetName(), uint32(getCFSRace()), uint32(GetClass()));
        return false;
    }
...
void Player::SaveToDB(CharacterDatabaseTransaction trans, bool create /* = false */)
{
    ...
    if (create)
    {
        ...
        stmt->setUInt32(index++, m_extraBonusTalentCount);
    }
    else
    {
        // Update query
...
uint32 Player::GetMaxPersonalArenaRatingRequirement(uint32 minarenaslot) const
{
    ...
    {
        if (i == 2 && sGameConfig->GetBoolConfig("Arena.1v1.VendorRating") == false) continue;
...
void Player::LearnCustomSpells()
{
    ...
    for (PlayerCreateInfoSpells::const_iterator itr = info->customSpells.begin(); itr != info->customSpells.end(); ++itr)
    {
        ...
        TC_LOG_DEBUG("entities.player.loading", "Player::LearnCustomSpells: Player '{}' ({}, Class: {} Race: {}): Adding initial spell (SpellID: {})",
            GetName(), GetGUID().ToString(), uint32(GetClass()), uint32(getCFSRace()), tspell);
...
void Player::LearnDefaultSkill(uint32 skillId, uint16 rank)
{
    ...
    TC_LOG_DEBUG("entities.player.loading", "PLAYER (Class: {} Race: {}): Adding initial skill, id = {}", uint32(GetClass()), uint32(getCFSRace()), skillId);
