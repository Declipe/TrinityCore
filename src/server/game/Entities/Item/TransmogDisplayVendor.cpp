...
bool TransmogDisplayVendorMgr::SuitableForTransmogrification(Player* player, ItemTemplate const* proto)
{
    ...
    if (player)
    {
        if ((proto->Flags[1] & ITEM_FLAG2_FACTION_HORDE) && player->GetTeam() != HORDE)
            return false;

        if ((proto->Flags[1] & ITEM_FLAG2_FACTION_ALLIANCE) && player->GetTeam() != ALLIANCE)
            return false;
...
const char * TransmogDisplayVendorMgr::getQualityName(uint32 quality)
{
    switch (quality)
    {
        ...
        case ITEM_QUALITY_UNCOMMON: return "|CFF006400[Uncommon]";
        case ITEM_QUALITY_RARE: return "|CFF00008b[Rare]";
        case ITEM_QUALITY_EPIC: return "|CFF4b0082[Epic]";
        case ITEM_QUALITY_LEGENDARY: return "|CFFff8c00[Legendary]";
