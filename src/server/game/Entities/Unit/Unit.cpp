...
void Unit::Update(uint32 p_time)
{
    ...
    {
        if (!GetThreatManager().GetAnyTarget())
        {
...
void Unit::CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    if (!this->GetGUID().IsPlayer() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
    ...
    if (Creature* bot = ToCreature())
        if (bot->GetIAmABot() || bot->GetIAmABotsPet())
            bot->ApplyBotDamageMultiplierMelee(damageInfo->Damages[0].Damage, *damageInfo);
    //End NpcBot
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    else if (!this->GetGUID().IsPlayer() && ToCreature()->GetBotAI())
    {
        DamageInfo dmgInfo(*damageInfo);
...
/*static*/ uint32 Unit::CalcArmorReducedDamage(Unit const* attacker, Unit* victim, uint32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType /*= MAX_ATTACK*/, uint8 attackerLevel /*= 0*/)
{
    ...
    if (!attacker->GetGUID().IsPlayer() && attacker->GetTypeId() == TYPEID_UNIT && attacker->ToCreature()->GetBotAI())
        armor *= std::max<float>(1.0f - attacker->ToCreature()->GetCreatureArmorPenetrationCoef(), 0.0f);
    //end npcbot
...
/*static*/ float Unit::CalculateAverageResistReduction(WorldObject const* caster, SpellSchoolMask schoolMask, Unit const* victim, SpellInfo const* spellInfo)
{
    ...
    if (caster)
    {
        ...
        if (Player const* player = caster->GetSpellModOwner())
        {
            ...
            if (!caster->GetGUID().IsPlayer() && caster->ToCreature()->GetBotAI())
                victimResistance -= float(caster->ToCreature()->GetCreatureSpellPenetration());
            //end npcbot
        }
...
void Unit::ExecuteDelayedSwingHit(bool extra)
{
    ...
    // ProcDamageAndSpell(_damageInfo.Target, _damageInfo.ProcAttacker, _damageInfo.ProcVictim, _damageInfo.ProcEx, _damageInfo.Damage, _damageInfo.AttackType); // Needs more checks to be correct
}
//NpcBot mod
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    if (!this->GetGUID().IsPlayer() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
    {
        ...
        else
        {
            ...
            CalculateMeleeDamage(victim, &_damageInfo, attType);
            DealDamageMods(victim, _damageInfo.Damages[0].Damage, &_damageInfo.Damages[0].Absorb);
...
float Unit::GetUnitDodgeChance(WeaponAttackType attType, Unit const* victim) const
{
    ...
    if (!this->GetGUID().IsPlayer() && ToCreature()->GetBotAI())
    {
        if (!this->GetGUID().IsPlayer() && !ToCreature()->CanDodge())
            return 0.0f;
...
float Unit::GetUnitParryChance(WeaponAttackType attType, Unit const* victim) const
{
    ...
    else
    {
        if (!victim->IsTotem() && !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY))
        {
            ...
            if (ToCreature() && ToCreature()->GetBotAI())
            {
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype) const
{
    ...
    if (ToCreature() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (schoolMask && !(schoolMask & SPELL_SCHOOL_MASK_NORMAL) && ToCreature() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
...
float Unit::SpellCritChanceTaken(Unit const* caster, SpellInfo const* spellInfo, SpellSchoolMask schoolMask, float doneChance, WeaponAttackType attackType /*= BASE_ATTACK*/, bool isPeriodic /*= false*/) const
{
    ...
    switch (spellInfo->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (spellInfo->SpellIconID == 2856 && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
            {
                ...
                if (!roll_chance_f(spellInfo->ProcChance * (isWrathSpell ? 0.6f : 1.0f)))
                    return false;

                // Unit* target = this;
                if (this->HasAura(isWrathSpell ? 48517 : 48518))
                    return false;

                uint32 triggered_spell_id = isWrathSpell ? 48518 : 48517;
    ...
    if (ToCreature() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
        ToCreature()->ApplyBotCritMultiplierAll(GetVictim(), crit_chance, spellInfo, schoolMask, attackType);
    //end npcbot
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint8 effIndex, Optional<float> const& donePctTotal, uint32 stack /*= 1*/) const
{
    ...
    if (ToCreature() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
        ToCreature()->ApplyBotDamageMultiplierHeal(GetVictim(), heal, spellProto, damagetype, stack);
    //end npcbot
...
bool Unit::IsImmunedToSpell(SpellInfo const* spellInfo, WorldObject const* caster) const
{
    ...
    if (ToCreature() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI() && ToCreature()->IsCreatureImmuneToSpell(spellInfo))
        return true;
    //end npcbot
    ...
    if (typeMask & MELEE_BASED_TRIGGER_MASK && procTarget)
    {
        ...
        if (hitMask & (PROC_HIT_CRITICAL | PROC_HIT_PARRY | PROC_HIT_DODGE | PROC_HIT_BLOCK))
        {
            ...
            if (isVictim)
            {
                ...
                if (ToCreature() && (hitMask & (PROC_HIT_DODGE | PROC_HIT_PARRY)) && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI() && ToCreature()->GetBotClass() == CLASS_WARRIOR)
                {
...
void Unit::ApplyPercentModFloatVar(float& var, float val, bool apply)
{
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/)
{
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        if (player || attacker->ToCreature() && !attacker->ToCreature()->GetBotAI() || attacker->ToCreature() && attacker->ToCreature()->GetBotOwner()->GetGUID().GetCounter() == attacker->GetGUID().GetCounter())
        //end npcbot
        {
...
float Unit::MeleeSpellMissChance(Unit const* victim, WeaponAttackType attType, int32 skillDiff, uint32 spellId) const
{
    ...
    if (ToCreature() && victim->GetTypeId() == TYPEID_UNIT && victim->ToCreature()->GetBotAI())
        evasion = victim->ToCreature()->GetCreatureEvasion();
    if (ToCreature() && GetTypeId() == TYPEID_UNIT && !ToCreature()->CanMiss())
        return evasion;
    if (ToCreature() && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI())
...
bool Unit::IsHighestExclusiveAura(Aura const* aura, bool removeOtherAuraApplications /*= false*/)
{
    ...
    // if (aura->GetCaster()->ToCreature()->GetBotAI())
    //    return false;
    //end npcbot
