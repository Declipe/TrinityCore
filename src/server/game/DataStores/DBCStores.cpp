...
void DBCMgr::LoadSpelldbcListStore()
{
    uint32 oldMSTime = getMSTime();
    //                                                0
    QueryResult result = ZynDatabase.Query("SELECT ID, "
        "Category, "    // 1
        "DispelType, "  // 2
        "Mechanic, "    // 3
        "Attributes, "  // 4
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 dbc_spell. DB table `dbc_spell` is empty.");
        return;
    }
    ...
    do
    {
        ...
        uint32 id = fields[0].GetUInt32();
        SpelldbcListEntry* SpelldbcList = new SpelldbcListEntry;
        SpelldbcList->ID = id;
        SpelldbcList->Category = fields[1].GetUInt32();
        SpelldbcList->DispelType = fields[2].GetUInt32();
        SpelldbcList->Mechanic = fields[3].GetUInt32();
        SpelldbcList->Attributes = fields[4].GetUInt32();
        SpelldbcList->AttributesEx = fields[5].GetUInt32();
        SpelldbcList->AttributesExB = fields[6].GetUInt32();
        SpelldbcList->AttributesExC = fields[7].GetUInt32();
        SpelldbcList->AttributesExD = fields[8].GetUInt32();
        SpelldbcList->AttributesExE = fields[9].GetUInt32();
        SpelldbcList->AttributesExF = fields[10].GetUInt32();
        SpelldbcList->AttributesExG = fields[11].GetUInt32();
        for (uint8 i = 0; i < 2; i++)
            SpelldbcList->ShapeshiftMask[i] = fields[12 + i].GetUInt32();
        for (uint8 i = 0; i < 2; i++)
            SpelldbcList->ShapeshiftExclude[i] = fields[14 + i].GetUInt32();
        SpelldbcList->Targets = fields[16].GetUInt32();
        SpelldbcList->TargetCreatureType = fields[17].GetUInt32();
        SpelldbcList->RequiresSpellFocus = fields[18].GetUInt32();
        SpelldbcList->FacingCasterFlags = fields[19].GetUInt32();
        SpelldbcList->CasterAuraState = fields[20].GetUInt32();
        SpelldbcList->TargetAuraState = fields[21].GetUInt32();
        SpelldbcList->ExcludeCasterAuraState = fields[22].GetUInt32();
        SpelldbcList->ExcludeTargetAuraState = fields[23].GetUInt32();
        SpelldbcList->CasterAuraSpell = fields[24].GetUInt32();
        SpelldbcList->TargetAuraSpell = fields[25].GetUInt32();
        SpelldbcList->ExcludeCasterAuraSpell = fields[26].GetUInt32();
        SpelldbcList->ExcludeTargetAuraSpell = fields[27].GetUInt32();
        SpelldbcList->CastingTimeIndex = fields[28].GetUInt32();
        SpelldbcList->RecoveryTime = fields[29].GetUInt32();
        SpelldbcList->CategoryRecoveryTime = fields[30].GetUInt32();
        SpelldbcList->InterruptFlags = fields[31].GetUInt32();
        SpelldbcList->AuraInterruptFlags = fields[32].GetUInt32();
        SpelldbcList->ChannelInterruptFlags = fields[33].GetUInt32();
        SpelldbcList->ProcTypeMask = fields[34].GetUInt32();
        SpelldbcList->ProcChance = fields[35].GetUInt32();
        SpelldbcList->ProcCharges = fields[36].GetUInt32();
        SpelldbcList->MaxLevel = fields[37].GetUInt32();
        SpelldbcList->BaseLevel = fields[38].GetUInt32();
        SpelldbcList->SpellLevel = fields[39].GetUInt32();
        SpelldbcList->DurationIndex = fields[40].GetUInt32();
        SpelldbcList->PowerType = fields[41].GetUInt32();
        SpelldbcList->ManaCost = fields[42].GetUInt32();
        SpelldbcList->ManaCostPerLevel = fields[43].GetUInt32();
        SpelldbcList->ManaPerSecond = fields[44].GetUInt32();
        SpelldbcList->ManaPerSecondPerLevel = fields[45].GetUInt32();
        SpelldbcList->RangeIndex = fields[46].GetUInt32();
        SpelldbcList->Speed = fields[47].GetFloat();
        //uint32 ModalNextSpell; (UNUSED)
        SpelldbcList->CumulativeAura = fields[48].GetUInt32();
        for (uint8 i = 0; i < 2; i++)
            SpelldbcList->Totem[i] = fields[49 + i].GetUInt32();
        for (uint8 i = 0; i < MAX_SPELL_REAGENTS; i++)
            SpelldbcList->Reagent[i] = fields[51 + i].GetInt32();
        for (uint8 i = 0; i < MAX_SPELL_REAGENTS; i++)
            SpelldbcList->ReagentCount[i] = fields[59 + i].GetInt32();

        SpelldbcList->EquippedItemClass = fields[67].GetInt32();
        SpelldbcList->EquippedItemSubclass = fields[68].GetInt32();
        SpelldbcList->EquippedItemInvTypes = fields[69].GetInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->Effect[i] = fields[70 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectDieSides[i] = fields[73 + i].GetInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectRealPointsPerLevel[i] = fields[76 + i].GetFloat();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectBasePoints[i] = fields[79 + i].GetInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectMechanic[i] = fields[82 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectImplicitTargetA[i] = fields[85 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectImplicitTargetB[i] = fields[88 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectRadiusIndex[i] = fields[91 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectAura[i] = fields[94 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectAuraPeriod[i] = fields[97 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectAmplitude[i] = fields[100 + i].GetFloat();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectChainTargets[i] = fields[103 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectItemType[i] = fields[106 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectMiscValue[i] = fields[109 + i].GetInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectMiscValueB[i] = fields[112 + i].GetInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectTriggerSpell[i] = fields[115 + i].GetUInt32();

        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectPointsPerCombo[i] = fields[118 + i].GetFloat();
        ...
        // var1:
        //for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
        //    SpelldbcList->EffectSpellClassMask[i].Set(fields[121 + i].GetUInt32(), fields[124 + i].GetUInt32(), fields[127 + i].GetUInt32());
        // var2:
        SpelldbcList->EffectSpellClassMask[0].Set(fields[121].GetUInt32(), fields[122].GetUInt32(), fields[123].GetUInt32());
        SpelldbcList->EffectSpellClassMask[1].Set(fields[124].GetUInt32(), fields[125].GetUInt32(), fields[126].GetUInt32());
        SpelldbcList->EffectSpellClassMask[2].Set(fields[127].GetUInt32(), fields[128].GetUInt32(), fields[129].GetUInt32());

        for (uint8 i = 0; i < 2; i++)
            SpelldbcList->SpellVisualID[i] = fields[130 + i].GetUInt32();

        SpelldbcList->SpellIconID = fields[132].GetUInt32();
        SpelldbcList->ActiveIconID = fields[133].GetUInt32();
        SpelldbcList->SpellPriority = fields[134].GetUInt32();

        for (uint8 i = 0; i < TOTAL_LOCALES; i++)
            SpelldbcList->Name[i] = fields[135 + i].GetString();

        //uint32 Name_lang_mask; (UNUSED)
        for (uint8 i = 0; i < TOTAL_LOCALES; i++)
            SpelldbcList->NameSubtext[i] = fields[150 + i].GetString();
        ...
        SpelldbcList->ManaCostPct = fields[165].GetUInt32();
        SpelldbcList->StartRecoveryCategory = fields[166].GetUInt32();
        SpelldbcList->StartRecoveryTime = fields[167].GetUInt32();
        SpelldbcList->MaxTargetLevel = fields[168].GetUInt32();
        SpelldbcList->SpellClassSet = fields[169].GetUInt32();
        SpelldbcList->SpellClassMask.Set(fields[170].GetUInt32(), fields[171].GetUInt32(), fields[172].GetUInt32());
        SpelldbcList->MaxTargets = fields[173].GetUInt32();
        SpelldbcList->DefenseType = fields[174].GetUInt32();
        SpelldbcList->PreventionType = fields[175].GetUInt32();
        //uint32 StanceBarOrder; (UNUSED)
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectChainAmplitude[i] = fields[176 + i].GetFloat();

        //uint32 MinFactionID; (UNUSED)
        //uint32 MinReputation; (UNUSED)
        //uint32 RequiredAuraVision; (UNUSED)
        for (uint8 i = 0; i < 2; i++)
            SpelldbcList->RequiredTotemCategoryID[i] = fields[179].GetUInt32();

        SpelldbcList->RequiredAreasID = fields[181].GetUInt32();
        SpelldbcList->SchoolMask = fields[182].GetUInt32();
        SpelldbcList->RuneCostID = fields[183].GetUInt32();
        //uint32 SpellMissileID; (UNUSED)
        //uint32 PowerDisplayID; (UNUSED)
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
            SpelldbcList->EffectBonusCoefficient[i] = fields[184 + i].GetFloat();
    ...
    TC_LOG_ERROR("server.loading", ">> Loaded DBC_spell                         {} in {} ms", (unsigned long)SpelldbcListStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
