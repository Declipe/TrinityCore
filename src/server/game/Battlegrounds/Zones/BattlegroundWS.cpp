...
void BattlegroundWS::EventPlayerCapturedFlag(Player* player)
{
    ...
    if (player->GetTeam() == ALLIANCE)
    {
        ...
        if (GetTeamScore(TEAM_ALLIANCE) < unsigned(sGameConfig->GetIntConfig("Battleground.Warsong.Flags")))
            AddPoint(ALLIANCE, 1);
    ...
    else
    {
        ...
        if (GetTeamScore(TEAM_HORDE) < unsigned(sGameConfig->GetIntConfig("Battleground.Warsong.Flags")))
            AddPoint(HORDE, 1);
