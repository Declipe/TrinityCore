...
bool bot_ai::GlobalUpdate(uint32 diff)
{
    ...
    if (_updateTimerEx1 <= diff && !IAmFree())
    {
        ...
        */
        ...
        //Looting
        if (me->IsInWorld() && HasRole(BOT_ROLE_AUTOLOOT) && (_roleMask & BOT_ROLE_MASK_LOOTING) &&
            !me->GetVictim() && !master->IsMounted() && !CCed(me) && !me->isMoving() && !Feasting() && !IsCasting() &&
            !HasBotCommandState(BOT_COMMAND_MASK_UNMOVING))
        {
            ...
            if (!crList.empty())
            {
                ...
                else
                {
                    ...
                    Position pos;
                    un->GetNearPoint(me, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 0.f, CONTACT_DISTANCE, un->GetAngle(me));
                    me->GetMotionMaster()->MovePoint(me->GetMapId(), pos, false);
                }*/
            }
        }
