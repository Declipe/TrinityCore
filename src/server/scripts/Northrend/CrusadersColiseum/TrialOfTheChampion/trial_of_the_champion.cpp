...
class npc_announcer_toc5 : public CreatureScript
{
    ...
    struct npc_announcer_toc5AI : public ScriptedAI
    {
        ...
        {
            if (bNextStep)
                //events.ScheduleEvent(uint32 eventId, Milliseconds time, uint32 group = 0, uint8 phase = 0)
                events.ScheduleEvent(currentEvent + 1, 0s, uiTimerStep);
            else
            {
                if (uiPhaseStep > 0)
                    events.ScheduleEvent(uiPhaseStep, 0s, uiTimerStep);
            }
        }
                ...
                {// && 
                    case EVENT_CHEER_RND:
                       // if (events.GetTimeUntilEvent -0s && !me->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP) && !me->isMoving() && !me->HasAura(66804))
                        if (events.GetTimeUntilEvent(!me->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP) && !me->isMoving() && !me->HasAura(66804)) == 0s)//;
                        {
