...
#define MSG_GOSSIP_SELL          "Продать дом гильдии"
#define MSG_GOSSIP_NEXTPAGE      "Далее -->"
#define MSG_INCOMBAT             "Вы в режиме боя."
#define MSG_NOGUILDHOUSE         "Ваша гильдия не владеет домом гильдии."
#define MSG_NOFREEGH             "Все дома гильдий заняты."
#define MSG_ALREADYHAVEGH        "Вы уже владеете домом гильдии (%s)."
#define MSG_NOTENOUGHMONEY       "У вас нехватает %u золота для покупки дома гильдии."
#define MSG_GHOCCUPIED           "Этот дом гильдии недоступен для покупки. Он уже занят."
#define MSG_CONGRATULATIONS      "Поздравляю! Вы успешно купили дом гильдии."
#define MSG_SOLD                 "Вы продали свой дом гильдии за %u золота."
#define MSG_NOTINGUILD           "Вы должны вступить в гильдию, чтобы использовать дом гильдии."
#define MSG_SELL_CONFIRM         "Вы уверены, что хотите продать дом гильдии?"
...
bool getGuildHouseCoords(uint32 guildId, float &x, float &y, float &z, uint32 &map)
{
 ...
 QueryResult result;
 result = ZynDatabase.PQuery("SELECT `x`, `y`, `z`, `map` FROM `guildhouses` WHERE `guildId` = %u", guildId);
 if (result)
 {
...
bool showBuyList(Player *player, Creature *_creature, uint32 showFromId = 0)
{
 ...
 QueryResult result;
 result = ZynDatabase.PQuery("SELECT `id`, `comment` FROM `guildhouses` WHERE `guildId` = 0 AND `id` > %u ORDER BY `id` ASC LIMIT %u",
...
{
 QueryResult result;
 result = ZynDatabase.PQuery("SELECT `comment` FROM `guildhouses` WHERE `guildId` = %u",
 player->GetGuildId());
...
void buyGuildhouse(Player *player, Creature *_creature, uint32 guildhouseId)
{
 ...
 result = ZynDatabase.PQuery("SELECT `id` FROM `guildhouses` WHERE `id` = %u AND `guildId` <> 0", guildhouseId);
 ...
 //update DB
 result = ZynDatabase.PQuery("UPDATE `guildhouses` SET `guildId` = %u WHERE `id` = %u",
