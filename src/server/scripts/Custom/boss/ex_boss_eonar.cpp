...
public:
    ...
    struct eonarAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            _events.ScheduleEvent(EVENT_BRAIN_LINK_DAMAGE, 60s);
            _events.ScheduleEvent(EVENT_MANGLING_SLASH, 8s);
            _events.ScheduleEvent(EVENT_SARGERAS, 10s);
            _events.ScheduleEvent(EVENT_TAIL_LASH, 5s);
            _events.ScheduleEvent(EVENT_SUMMONS, 30s);
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage) override
        {
            if (me->HealthBelowPctDamaged(65, damage) && _events.IsInPhase(PHASE_ONE))
            {
                ...
                _events.ScheduleEvent(EVENT_MANA_DESTRUCTION, 10s);
                _events.ScheduleEvent(EVENT_PIERCING_SLASH, 20s);
                _events.ScheduleEvent(EVENT_BRAIN_LINK_DAMAGE, 10s);
                _events.ScheduleEvent(EVENT_BLOOD_MIRROR_DAMAGE, 10s);
                _events.ScheduleEvent(EVENT_TAIL_LASH, 5s);
            ...
            if (me->HealthBelowPctDamaged(25, damage) && _events.IsInPhase(PHASE_TWO))
            {
                ...
                _events.ScheduleEvent(EVENT_CRYSTAL_CHAINS, 5s);
                _events.ScheduleEvent(EVENT_PIERCING_SLASH, 10s);
                _events.ScheduleEvent(EVENT_NECROTIC_POISON, 12s);
                _events.ScheduleEvent(EVENT_ANNOYING_YIPPING, 25s);
                _events.ScheduleEvent(EVENT_BURN, 35s);
                _events.ScheduleEvent(EVENT_TAIL_LASH, 5s);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_SUMMONS:
                    me->SummonCreature(NPC_ADD, me->GetPositionX() + 5, me->GetPositionY() + 5, me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 60s);
                    _events.ScheduleEvent(EVENT_SUMMONS, 30s);
                    break;
                ...
                case EVENT_SARGERAS:
                    ...
                    _events.ScheduleEvent(EVENT_SARGERAS, 5s);
                    break;
                case EVENT_BURN:
...
public: eonaradd() : CreatureScript("eonaradd") { }
        ...
        struct eonaraddAI : public ScriptedAI
        {
            ...
            void JustEngagedWith(Unit*) override
            {
                Talk(SAY_AGGRO);
                _events.SetPhase(PHASE_ONE);
                _events.ScheduleEvent(EVENT_EISBLOCK, 10s);
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& damage) override
            {
                if (me->HealthBelowPctDamaged(100, damage) && _events.IsInPhase(PHASE_ONE))
                {
                    ...
                    _events.ScheduleEvent(EVENT_EISBLOCK, 15s);
                }
