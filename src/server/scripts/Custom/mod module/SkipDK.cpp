...
void Trinitycore_skip_deathknight_HandleSkip(Player* player)
{
    ...
    int DKL = sGameConfig->GetFloatConfig("Skip.Deathknight.Start.Level", 58);
...
public:
    ...
    {
        if (sGameConfig->GetBoolConfig("Skip.Deathknight.Starter.Announce.enable", true))
        {
...
public:
    ...
    void OnLogin(Player* player, bool firstLogin) override
    {
        if (firstLogin && player->GetAreaId() == 4342)
        {
            ...
            if ((sGameConfig->GetBoolConfig("Skip.Deathknight.Starter.Enable", true) && player->GetSession()->GetSecurity() == SEC_PLAYER)
                || (sGameConfig->GetBoolConfig("GM.Skip.Deathknight.Starter.Enable", true) && player->GetSession()->GetSecurity() >= SEC_MODERATOR))
            {
...
public:
    ...
    struct npc_SkipLichAI : public ScriptedAI
    {
        ...
        bool OnGossipHello(Player* player) override
        {
            ...
            if (sGameConfig->GetBoolConfig("Skip.Deathknight.Optional.Enable", true))
            {
