...
{
    allowedStats.clear();
    std::vector<std::string_view> tokenized = Trinity::Tokenize(stats, ',', false);
    std::transform(tokenized.begin(), tokenized.end(), std::back_inserter(allowedStats),
        [](const std::string_view& str) { return *Trinity::StringTo<uint32>(str); });
}
...
void ItemUpgrade::LoadFromDB(bool reload)
{
    ...
    TC_LOG_INFO("server.loading", "Loading item upgrade mod custom tables...");
    ...
    {
        TC_LOG_ERROR("server.loading", "Found data validity errors while loading item upgrade mod tables. Check the FATAL error messages and fix the issues before attempting to restart the server");
...
void ItemUpgrade::LoadAllowedItems()
{
    ...
    QueryResult result = ZynDatabase.Query("SELECT entry FROM mod_item_upgrade_allowed_items");
    if (!result)
        return;
    ...
    do
    {
        ...
        if (!itemTemplate)
        {
            TC_LOG_ERROR("sql.sql", "Table `mod_item_upgrade_allowed_items` has invalid item entry {}, skip", entry);
            continue;
        }
...
void ItemUpgrade::LoadAllowedStatsItems()
{
    ...
    QueryResult result = ZynDatabase.Query("SELECT stat_id, entry FROM mod_item_upgrade_allowed_stats_items");
    if (!result)
        return;
    ...
    do
    {
        ...
        uint32 stat_id = fields[0].GetUInt32();
        uint32 entry = fields[1].GetUInt32();
        const ItemTemplate* itemTemplate = sObjectMgr->GetItemTemplate(entry);
        if (!itemTemplate)
        {
            TC_LOG_ERROR("sql.sql", "Table `mod_item_upgrade_allowed_stats_items` has invalid item entry {}, skip", entry);
            continue;
        }
...
void ItemUpgrade::LoadBlacklistedItems()
{
    ...
    QueryResult result = ZynDatabase.Query("SELECT entry FROM mod_item_upgrade_blacklisted_items");
    if (!result)
        return;
    ...
    do
    {
        ...
        if (!itemTemplate)
        {
            TC_LOG_ERROR("sql.sql", "Table `mod_item_upgrade_blacklisted_items` has invalid item entry {}, skip", entry);
            continue;
        }
...
void ItemUpgrade::LoadBlacklistedStatsItems()
{
    ...
    QueryResult result = ZynDatabase.Query("SELECT stat_id, entry FROM mod_item_upgrade_blacklisted_stats_items");
    if (!result)
        return;
    ...
    do
    {
        ...
        if (!itemTemplate)
        {
            TC_LOG_ERROR("sql.sql", "Table `mod_item_upgrade_blacklisted_stats_items` has invalid item entry {}, skip", entry);
            continue;
        }
...
void ItemUpgrade::CleanupDB(bool reload)
{
    ...
    ZynDatabaseTransaction trans = ZynDatabase.BeginTransaction();
    ...
    CharacterDatabase.DirectCommitTransaction(trans2);
}
...
void ItemUpgrade::MergeStatRequirements(std::unordered_map<uint32, StatRequirementContainer>& statRequirementMap)
{
    for (auto& statPair : statRequirementMap)
    {
        ...
        if (copperTotal > 0.0f)
        {
            ...
            if (val < 1 || unsigned(val) > MAX_MONEY_AMOUNT)
                TC_LOG_ERROR("sql.sql", "Stat requirement has invalid total copper amount for stat id {}, skip", statPair.first);
            else
        ...
        if (honorTotal > 0.0f)
        {
            ...
            if (val < 1 || unsigned(val) > sWorld->getIntConfig(CONFIG_MAX_HONOR_POINTS))
                TC_LOG_ERROR("sql.sql", "Stat requirement has invalid total honor points for stat id {}, skip", statPair.first);
            else
        ...
        if (arenaTotal > 0.0f)
        {
            ...
            if (val < 1 || unsigned(val) > sWorld->getIntConfig(CONFIG_MAX_ARENA_POINTS))
                TC_LOG_ERROR("sql.sql", "Stat requirement has invalid total arena points for stat id {}, skip", statPair.first);
            else
...
void ItemUpgrade::LoadStatRequirements()
{
    ...
    QueryResult result = ZynDatabase.Query("SELECT id, stat_id, req_type, req_val1, req_val2 FROM mod_item_upgrade_stats_req");
    if (!result)
        return;
    ...
    do
    {
        ...
        uint32 statId = fields[1].GetUInt32();
        uint8 reqType = fields[2].GetUInt8();
        if (!IsValidReqType(reqType))
        {
            TC_LOG_ERROR("sql.sql", "Table `mod_item_upgrade_stats_req` has invalid `req_type` {}, skip", reqType);
            continue;
        }
        float reqVal1 = fields[3].GetFloat();
        float reqVal2 = fields[4].GetFloat();
        if (!ValidateReq(fields[0].GetUInt32(), (UpgradeStatReqType)reqType, reqVal1, reqVal2, "mod_item_upgrade_stats_req"))
            continue;
...
void ItemUpgrade::LoadStatRequirementsOverrides()
{
    ...
    QueryResult result = ZynDatabase.Query("SELECT id, stat_id, item_entry, req_type, req_val1, req_val2 FROM mod_item_upgrade_stats_req_override");
    if (!result)
        return;
    ...
    do
    {
        ...
        uint32 statId = fields[1].GetUInt32();
        uint8 reqType = fields[3].GetUInt8();
        if (!IsValidReqType(reqType))
        {
            TC_LOG_ERROR("sql.sql", "Table `mod_item_upgrade_stats_req_override` has invalid `req_type` {}, skip", reqType);
            continue;
        }
        uint32 entry = fields[2].GetUInt32();
        const ItemTemplate* proto = sObjectMgr->GetItemTemplate(entry);
        if (proto == nullptr) {
            TC_LOG_ERROR("sql.sql", "Table `mod_item_upgrade_stats_req_override` has invalid `item_entry` {}, skip", entry);
            continue;
        }
        float reqVal1 = fields[4].GetFloat();
        float reqVal2 = fields[5].GetFloat();
        if (!ValidateReq(fields[0].GetUInt32(), (UpgradeStatReqType)reqType, reqVal1, reqVal2, "mod_item_upgrade_stats_req_override"))
            continue;
...
void ItemUpgrade::LoadUpgradeStats()
{
    ...
    QueryResult result = ZynDatabase.Query("SELECT id, stat_type, stat_mod_pct, stat_rank FROM mod_item_upgrade_stats");
    if (!result)
        return;
    ...
    do
    {
        ...
        uint32 statType = fields[1].GetUInt32();
        float statModPct = fields[2].GetFloat();
        uint16 statRank = fields[3].GetUInt16();
...
void ItemUpgrade::LoadCharacterUpgradeData()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 character item upgrades.");
        TC_LOG_INFO("server.loading", " ");
        return;
    }
    ...
    do
    {
        ...
        ObjectGuid itemGuid = ObjectGuid::Create<HighGuid::Item>(fields[1].GetUInt32());
        uint32 statId = fields[2].GetUInt32();
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `character_item_upgrade` has invalid `stat_id` {}, this should never happen, skip", statId);
            continue;
        }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded {} character item upgrades in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    TC_LOG_INFO("server.loading", " ");
}
...
bool ItemUpgrade::_AddPagedData(Player* player, const PagedData& pagedData, uint32 page) const
{
    ...
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Total items upgraded: " + Trinity::ToString(pagedData.data.size()), GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Total upgrades: " + Trinity::ToString(totalUpgrades), GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
    }
    ...
    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<- [Back]", page == 0 ? pageZeroSender : GOSSIP_SENDER_MAIN + 2, page == 0 ? static_cast<uint32>(GOSSIP_ACTION_INFO_DEF) : static_cast<uint32>(GOSSIP_ACTION_INFO_DEF) + page - 1);
...
bool ItemUpgrade::HandlePurchaseRank(Player* player, Item* item, const UpgradeStat* upgrade)
{
    ...
    if (foundUpgrade != nullptr)
    {
        ...
        CharacterDatabase.PExecute("UPDATE character_item_upgrade SET stat_id = {} WHERE guid = {} AND item_guid = {} AND stat_id = {}",
...
void ItemUpgrade::RemoveItemUpgrade(Player* player, Item* item)
{
    ...
    CharacterDatabase.PExecute("DELETE FROM character_item_upgrade WHERE guid = {} AND item_guid = {}", player->GetGUID().GetCounter(), item->GetGUID().GetCounter());
}
...
void ItemUpgrade::BuildRequirementsPage(const Player* player, PagedData& pagedData, const StatRequirementContainer* reqs) const
{
    ...
    else
    {
        for (const auto& req : *reqs)
        {
            ...
            Identifier* identifier = new Identifier();
            identifier->id = 0;
            identifier->name = Trinity::ToString<uint32>((uint32)req.reqType);
...
bool ItemUpgrade::ChooseRandomUpgrade(Player* player, Item* item)
{
    ...
    Trinity::Containers::RandomShuffle(upgrades);
...
{
    CharacterDatabase.PExecute("INSERT INTO character_item_upgrade (guid, item_guid, stat_id) VALUES ({}, {}, {})",
...
bool ItemUpgrade::CheckDataValidity() const
{
    ...
    for (const UpgradeStat& upgrade : upgradeStatList)
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "FATAL: Table `mod_item_upgrade_stats` has invalid `stat_type` {}", upgrade.statType);
            ok = false;
        }
        if (upgrade.statModPct <= 0)
        {
            TC_LOG_ERROR("sql.sql", "FATAL: Table `mod_item_upgrade_stats` has invalid `stat_mod_pct` {}", upgrade.statModPct);
            ok = false;
        }
    }
    ...
    for (auto& rpair : ranksMap)
    {
        ...
        {
            ok = false;
            TC_LOG_ERROR("sql.sql", "FATAL: Table `mod_item_upgrade_stats` has invalid starting rank (`stat_rank`) {} for stat type (`stat_type`) {}", ranks[0], rpair.first);
        }
        ...
        {
            ok = false;
            TC_LOG_ERROR("sql.sql", "FATAL: Table `mod_item_upgrade_stats` does not have consecutive ranks (`stat_rank`) for stat type (`stat_type`) {}", rpair.first);
        }
    }
