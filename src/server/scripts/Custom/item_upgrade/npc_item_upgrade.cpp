...
public:
    ...
    struct npc_item_upgradeAI : public ScriptedAI
    {
        npc_item_upgradeAI(Creature* me) : ScriptedAI(me) { }
    ...
    bool GossipSelect(Player* player, uint32 sender, uint32 action)
    {
        ...
        else if (sender == GOSSIP_SENDER_MAIN + 1)
        {
            ...
            return sItemUpgrade->TakePagedDataAction(player, me, id);
        }
        else if (sender == GOSSIP_SENDER_MAIN + 2)
        {
            uint32 page = action - GOSSIP_ACTION_INFO_DEF;
            return sItemUpgrade->AddPagedData(player, me, page);
        }
...
{
    return new npc_item_upgradeAI(me);
}
};
